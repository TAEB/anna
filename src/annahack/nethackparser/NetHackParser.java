...
public class NetHackParser
{
	...
	private boolean parseStatusLine() throws IOException
	{
		...
		try
		{
			...
			pw.setDx(Integer.parseInt(m_dx.group()));
			pw.setIn(Integer.parseInt(m_in.group()));
			pw.setWi(Integer.parseInt(m_wi.group()));
			pw.setCh(Integer.parseInt(m_ch.group()));
			
			pw.setAlignment(m_align.group());
			
			score=Integer.parseInt(m_score.group());
			//End top line
			
			//Begin bottom line
			//TODO: Add gold to inventory
			dlvl= Integer.parseInt(m_dlvl.group());
			turn= Integer.parseInt(m_turn.group());
			...
			pw.setAC(Integer.parseInt(m_mp.group()));
